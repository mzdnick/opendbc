...
class CarState(CarStateBase, CarStateExt):
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    # Unfiltered, both longitudinal modes: the TJA-press cleanup and the white-wheel HUD gate read it.
    ...
    # the declaration because an undeclared wheel's BIT1 idle level is unknown, and held
    # unpressed until the wheel's first frame: the parser reads zeros before it, which the
    # active-low decode would turn into a phantom press at startup.
