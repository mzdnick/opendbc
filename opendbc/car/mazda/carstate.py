...
LAT_BLOCK_CONFIRM_FRAMES = round(0.2 / DT_CTRL)
...
class CarState(CarStateBase, CarStateExt):
  ...
  def update_steer_undelivered(self, v_ego_raw: float, lkas_request: float) -> None:
    ...
    # Dash LKA off also clears the latch.
    ...
    if self.steer_undelivered:
      ...
      # The latch can outlive the request.
      self.steer_undelivered_frames += 1
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    # LKAS_BLOCK also covers standstill and the ~3 s re-arm after a re-enable:
    # the UI shows arming until the EPS grants lateral.
    ...
    # LANE_LINES is the state of the dash LKA button
      ...
      if len(lane_lines_vals) > 0:
        ...
        if lkas_on != self.lkas_on_candidate:
          ...
          if not self.CP_SP.flags & MazdaFlagsSP.TJA_BUTTON:
            self.lkas_button = 1
      # the state is not the switch: a TJA car publishes it too
