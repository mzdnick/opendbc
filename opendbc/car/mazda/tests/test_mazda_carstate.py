...
from opendbc.car.mazda.carstate import ButtonType, CAM_LANEINFO_FRESH_FRAMES, LAT_BLOCK_CONFIRM_FRAMES, STOCK_CTS_ALERT_FRAMES
...
class TestLkaButtonToggle:
  ...
  def test_only_declared_platforms_read_the_toggle(self):
    # every platform declares the toggle now; a cleared flag stands in for one that does not
    CI, pk = car_interface(alpha_long=False), packer()
    CI.CP_SP.flags &= ~MazdaFlagsSP.LKA_BUTTON
