...
class TestMazdaEpsSwap:
  ...
  def test_foreign_radar_runs_vision_only(self):
    # a first-gen body keeps its own radar (real case: G46L-67XA1-C, firmware in
    # no platform list, sends no track messages) — without this it starves
    # radarTracks and every radar-dependent validity check fails
  ...
  def test_known_radar_stays_available(self):
    # the 2017-21 KF radar is a different platform's entry but the same track
    # dialect: a list anywhere keeps the radar on
