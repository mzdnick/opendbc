...
// Lane-keep line count, low 3 bits of byte 1 (Motorola bits 10..8): 0 with the dash button off, 1-4 lane state.
...
// Declared by the platform: the dash LKA button owns lateral and the HUD frame is checked.
...
static void mazda_rx_hook(const CANPacket_t *msg) {
  ...
  // The dash LKA button has no CAN signal: LANE_LINES is its state (0 = off). Only the on-press is
  // reported: a press cannot exit controls, the off-toggle is the software's to act on. Two agreeing frames.
