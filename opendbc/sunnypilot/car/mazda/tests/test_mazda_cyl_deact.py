...
conditions, and the hold for modes never observed on the wire.
"""
import pytest
...
def feed_mode(CI, i: int, mode: int):
  """Three frames so the parser has decoded the message before the assert."""
  for j in range(3):
...
def test_fuel_cut_inference_needs_confirm_time(CI):
  ...
  # every frame of the countdown stays normal; only the confirm frame flips
...
def test_coupling_breaks_on_gear_change_and_resumes(CI):
  ...
  # every frame is asserted so a mid-loop flicker cannot pass silently
  prime_coupling(CI.CS)
  for _ in range(EB_CONFIRM_FRAMES + 5):
    CI.CS.update_cylinder_deactivation(*frame())
  assert CI.CS.cyl_state == CylinderState.engineBraking
  new_line = LINE - 6.0
  # the p95-p5 gate is rank-based: the old line holds through the first p5-rank frames,
  # the mixed window breaks coupling until frame p95 (too few old samples left to move
  # the 95th rank), and the confirm then completes EB_CONFIRM_FRAMES coupled frames
  p5 = EB_LOCK_FRAMES * 5 // 100
  p95 = EB_LOCK_FRAMES * 95 // 100
  for i in range(EB_LOCK_FRAMES + EB_CONFIRM_FRAMES):
    CI.CS.update_cylinder_deactivation(*frame(ratio=new_line))
    expected = (CylinderState.engineBraking
                if i < p5 or i - p95 + 1 >= EB_CONFIRM_FRAMES
                else CylinderState.normal)
    assert CI.CS.cyl_state == expected
...
def test_unobserved_modes_hold_state(CI, mode):
  # 0x12/0x13 sit between the ramp top and the latch and never appeared in the capture;
  # the byte was observed jumping 0x11 straight to 0x14. A value with no mapping carries
  # no information: the resolved state and its ramp hold through that frame.
  CI.CS.update_cylinder_deactivation(MODE_ENTRY_MID, gas_pressed=True, v_ego=20.0,
                                     rpm=LINE * KPH, speed_kph=KPH)
  assert CI.CS.cyl_state == CylinderState.entry
  CI.CS.update_cylinder_deactivation(mode, gas_pressed=True, v_ego=20.0,
                                     rpm=LINE * KPH, speed_kph=KPH)
  assert CI.CS.cyl_state == CylinderState.entry
  assert CI.CS.cyl_entry_progress == pytest.approx(3 / 7)
  CI.CS.update_cylinder_deactivation(CYL_MODE_DEACTIVATED, gas_pressed=True, v_ego=20.0,
                                     rpm=LINE * KPH, speed_kph=KPH)
  assert CI.CS.cyl_state == CylinderState.deactivated
  CI.CS.update_cylinder_deactivation(mode, gas_pressed=True, v_ego=20.0,
                                     rpm=LINE * KPH, speed_kph=KPH)
  assert CI.CS.cyl_state == CylinderState.deactivated
  assert CI.CS.cyl_entry_progress == 1.0
...
def test_coupling_band_scales_with_the_gear_line(CI):
  # the spread gate is a fraction of the window median: the same wobble is judged the
  # same at a first-gear line (about 105 rpm/kph) as at the cruise line, where one
  # absolute rpm/kph band would be tight in low gears and loose in high ones
  def couples(wobble: float, base: float) -> bool:
    CI.CS.eb_ratio_hist.clear()
    CI.CS.eb_confirm_frames = 0
    for i in range(EB_LOCK_FRAMES + EB_CONFIRM_FRAMES):
      CI.CS.update_cylinder_deactivation(*frame(ratio=base + (wobble if i % 2 else -wobble)))
    return CI.CS.cyl_state == CylinderState.engineBraking

  assert couples(0.0, LINE)            # a flat cruise line couples
  assert couples(3.0, 105.0)           # low gear: the same 3 rpm/kph wobble still couples
  assert not couples(3.0, LINE)        # cruise line: a 10% wobble does not
